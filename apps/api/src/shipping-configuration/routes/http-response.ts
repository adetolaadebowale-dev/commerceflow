...
import { ShippingMethodError, ShippingZoneError } from "../errors";
...
export function handleShippingConfigurationRouteError(
  error: unknown,
): Response {
  ...
  if (error instanceof ShippingZoneError || error instanceof ShippingMethodError) {
    return jsonError(
      {
        code: error.code,
