...
import { DisasterReadinessError } from "../errors";
...
export function handleDisasterReadinessRouteError(error: unknown): Response {
  const known = respondKnownRouteError(error);
  if (known) {
    return known;
  }

  if (error instanceof DisasterReadinessError) {
    return jsonError(
      {
        code: error.code,
